...
class MainActivity : AppCompatActivity() {
    ...
    // НАПИТОК представляет состояние "пить лимонад"
    private val DRINK = "drink"
    // RESTART represents the state where the lemonade has been drunk and the glass is empty
    // ПЕРЕЗАПУСК представляет состояние, в котором лимонад был выпит, а стакан пуст.
    private val RESTART = "restart"
    // Default the state to select
    private var lemonadeState = "select"
    // Default lemonSize to -1
    // Размер лимона по умолчанию равен -1
    private var lemonSize = -1
    // Default the squeezeCount to -1
    // По умолчанию количество сжатий равно -1
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lemonImage!!.setOnClickListener {
            clickLemonImage() // TODO: вызовите метод, который обрабатывает состояние при щелчке по изображению
        }
     ...
     *
     * Этот метод сохраняет состояние приложения, если оно переведено в фоновый режим.
     */
     ...
     * Щелчок вызовет разный ответ в зависимости от состояния.
    ...
    private fun clickLemonImage() {
        when(lemonadeState) {
            ...
            SQUEEZE -> {
                squeezeCount ++
                lemonSize --
                if (lemonSize == 0) lemonadeState = DRINK else lemonadeState = SQUEEZE
            }
            DRINK -> {
        ...
        //используйте условное выражение типа "если" или "когда", чтобы отслеживать состояние лимонада
...
// случае выжимания лимона). Это должно быть сделано в этом условном выражении
        ...
        //Когда изображение щелкается в состоянии ВЫБОРА, состояние должно стать СЖАТЫМ
...
// - Количество выжиманий должно быть равно 0, так как мы еще не выжали ни одного лимона.
        ...
        // При нажатии на изображение в состоянии СЖАТИЯ количество сжатий должно быть
        ...
        // При нажатии на изображение в состоянии НАПИТКА состояние должно стать ПЕРЕЗАПУСКОМ

        // TODO: When the image is clicked in the RESTART state the state should become SELECT
        // При нажатии на изображение в состоянии ПЕРЕЗАПУСКА состояние должно стать SELECT


        // TODO: lastly, before the function terminates we need to set the view elements so that the
        //  UI can reflect the correct state
        // наконец, перед завершением функции нам нужно настроить элементы представления так, чтобы
    ...
    private fun setViewElements() {
        ...
        // настройте условие, которое отслеживает состояние лимонада

        // TODO: for each state, the textAction TextView should be set to the corresponding string from
        //  the string resources file. The strings are named to match the state
        // для каждого состояния textAction TextView должно быть установлено в соответствующую строку из
        // файл string resources. Строки названы в соответствии с состоянием

        // TODO: Additionally, for each state, the lemonImage should be set to the corresponding
        //  drawable from the drawable resources. The drawables have the same names as the strings
        //  but remember that they are drawables, not strings.
        // Кроме того, для каждого состояния изображение лимона должно быть установлено на соответствующее
     ...
     * Длительное нажатие на изображение лимона покажет, сколько раз лимон был выжат.
     */
